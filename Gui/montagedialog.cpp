    ...
    ui(new Ui::MontageDialog)
{
    ...
    ui->activeButton->setText(QStringLiteral("激活鸿合助手"));
    ui->installButton->setText(QStringLiteral("安装Montage"));
    ui->cancelButton->setText(QStringLiteral("取消"));
    ui->plainTextEdit->setPlainText(QStringLiteral("激活Montage失败，请手动添加到鸿合助手。\n如果没有安装,请先安装。"));
#else
    //ui->plainTextEdit->setPlaceholderText(tr("Can not active Montage. Please add Montage to Newline Assistant manually.\n If Montage is not installed.please install it first."));
#endif
