...
# if HHT_CHINA
    ...
    syncAction = new QAction(tr("手动同步"),this);
...
bool MainDialog::isPublicUSBStatus(QList<QString> deviceList)
{
    ...
    for(int i=0;i<deviceList.size();i++)
    {
        ...
        if(usbService=="USBSTOR")//usb设备是存储设备
        {
            ...
            else if(usbLocationInfo.contains("Hub_#0006")||usbLocationInfo.contains("Hub_#0007"))//国内
            {
                qDebug()<<"China Public USB Port is busy now..... ";
                HHT_LOG(EN_INFO, "China Public USB Port is busy now..... ");
                return true;
            }
        }
    }
...
void MainDialog::slot_openProcFromFileName(QString fileName)
{
    ...
    else
    {
        HintDialog *hint = new HintDialog();
#if HHT_CHINA
        hint->setMassage(QStringLiteral("从智能设备接收的指令为空 . "),-2);
#else
...
void MainDialog::slot_fileNameTooLong()
{
    HintDialog *hint = new HintDialog();
#if HHT_CHINA
    hint->setMassage(QStringLiteral("文件名过长，无法拖动。"),-2); //拖入项目字符长度超过限制
#else
